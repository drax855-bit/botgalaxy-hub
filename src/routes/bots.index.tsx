...
import { z } from "zod";
import { listBots, getCategories } from "@/lib/directory.functions";
...
const searchSchema = z.object({
  ...
  sort: z.enum(["popular", "newest", "top_rated", "servers", "votes"]).optional(),
...
export const Route = createFileRoute("/bots")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Search and filter every Discord bot in the BotGalaxy directory by category, rating and votes.",
      },
      { property: "og:title", content: "Explore Discord bots — BotGalaxy" },
      { property: "og:description", content: "Instant search across 22 Discord bot categories." },
    ],
  }),
  component: Browse,
});
...
function Browse() {
  ...
  const navigate = useNavigate({ from: "/bots" });
  ...
  const categories = useQuery({ queryKey: ["categories"], queryFn: () => getCategories(), staleTime: 300_000 });

  const page = search.page ?? 1;
  const params = {
    q: search.q ?? "",
    category: search.category ?? "",
    sort: search.sort ?? ("popular" as const),
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <h1 className="font-display text-3xl font-bold sm:text-4xl">Explore bots</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        {list.data ? `${list.data.total} bots match your filters` : "Loading directory…"}
      </p>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {(["verified", "premium", "featured"] as const).map((k) => (
          <Button key={k} size="sm" variant={search[k] ? "default" : "secondary"} onClick={() => toggle(k)}>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {list.isPending
          ? Array.from({ length: 8 }).map((_, i) => <BotCardSkeleton key={i} />)
          : (list.data?.items ?? []).map((b) => <BotCard key={b.id} bot={b} />)}
      </div>

      {list.data && list.data.items.length === 0 && (
        <div className="mt-16 text-center">
          <h2 className="font-display text-xl font-semibold">No bots found</h2>
          <p className="mt-2 text-sm text-muted-foreground">Try a different search term or clear your filters.</p>
          <Button asChild className="mt-4" variant="secondary">
            <Link to="/bots">Reset filters</Link>
          </Button>
        </div>
      )}
